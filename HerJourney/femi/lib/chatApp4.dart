...
class KeywordChatApp3 extends StatefulWidget {
  const KeywordChatApp3({super.key});

  @override
  _KeywordChatApp3State createState() => _KeywordChatApp3State();
}

class _KeywordChatApp3State extends State<KeywordChatApp3> {
  ...
  Future<void> _initializeDatabase() async {
    String path = join(await getDatabasesPath(), 'ai_chat4.db');
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.insert('responses', {
          'keyword': 'causes',
          'response':
              'It may cause by hormonal changes, lifestyle factors, medications and many other conditions like PCOS.'
        });
        await db.insert('responses',
            {'keyword': 'exercise', 'response': 'Daily yoga can help it'});
        await db.insert('responses', {
          'keyword': 'diet',
          'response':
              'Drink apple cider vinegar, pineapple must be added to your regular diet.'
        });
        await db.insert('responses', {
          'keyword': 'worry',
          'response':
              'there is nothing to worry if you want then please go see your doctor for further help.'
        });
      },
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Irregular cycles'),
      ),
      body: Column(
        children: [
